---
title: "Stat 343 MLE Practice: Leaf Margins"
output:
  pdf_document:
    keep_tex: yes
---
...
P_2 <- function(p) {
  loglikelihood(p_updated) + (540/p_updated + (540 - 2700)/(1 - p_updated)) * (p - p_updated) + (-540/(p_updated^2) + (540-2700)/((1 - p_updated)^2)) * (p - p_updated)^2
}
...
```{r}
p_old <- 0.9
p_updated <- p_old - (540/p_old + (540 - 2700)/(1 - p_old)) / (-540/(p_old^2) + (540-2700)/((1 - p_old)^2))
```
...
P_2 <- function(p) {
  loglikelihood(p_updated) + (540/p_updated + (540 - 2700)/(1 - p_updated)) * (p - p_updated) + (-540/(p_updated^2) + 0.5 * (540-2700)/((1 - p_updated)^2)) * (p - p_updated)^2
}

ggplot(data = data.frame(p = c(0, 1)), mapping = aes(x = p)) +
  stat_function(fun = loglikelihood, n = 1001) +
  stat_function(fun = P_2, color = "cornflowerblue") +
  geom_vline(xintercept = 0.2) +
